import pygame as pg, sys, random as rdm, time, tkinter #Temps pour le chrono et interface graphique, police, taille du texte-KV
from pygame.locals import *
from tkinter import * #-KV
from tkinter import messagebox#-KV
pg.init()
...
window=pg.display.set_mode((length,width),RESIZABLE)#rend l'écran redimensionnable-KV
...
while maintain:
    ...
    if scrolling/100>100: #Vérifie qu'on est pas à la fin du niveau
        print("Gagné !".upper())# conversion en majuscule-KV
        maintain=False
    ...
    for event in pg.event.get():
        if event.type==KEYDOWN and event.key==K_RIGHT: #Traite la flèche DROITE
            ...
            if player_pos_x==130: #Restriction des mouvements
                ...
                player_pos_x-=50
        elif event.type==KEYDOWN and event.key==K_SPACE : #Traite la flèche ESPACE
            speed +=1
            print (str(speed))
